...
namespace Foam
{
    defineTypeNameAndDebug(fsppControl, 0);
}


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

bool Foam::fsppControl::read()
{
    solutionControl::read(true);

    const dictionary fsppDict(dict());
    nCorrFSPP_ = fsppDict.getOrDefault<label>("nCorrectors", 1);
    return true;
}


bool Foam::fsppControl::criteriaSatisfied()
{
